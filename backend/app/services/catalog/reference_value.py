import uuid

from app.models.catalog.reference_value import ReferenceValue
from app.models.catalog.reference_value_translation import ReferenceValueTranslation
from app.repositories.catalog.reference_value import ReferenceValueRepository
from app.schemas.catalog.reference_value import ReferenceValueCreate, ReferenceValueUpdate
from app.services.base import BaseService
from app.services.translation import upsert_translations


class ReferenceValueService(
    BaseService[ReferenceValue, ReferenceValueCreate, ReferenceValueUpdate]
):
    entity_name = "Reference value"
    resource = "reference-values"
    search_fields = ["code", "reference_type"]

    def __init__(self, repository: ReferenceValueRepository) -> None:
        super().__init__(repository)
        self.repository: ReferenceValueRepository = repository

    async def create(self, obj_in: ReferenceValueCreate) -> ReferenceValue:
        data = obj_in.model_dump(exclude_unset=True)
        translations = data.pop("translations", None) or {}
        obj = await self.repository.create(data)
        if translations:
            await upsert_translations(
                self.repository.session,
                translation_model=ReferenceValueTranslation,
                parent_fk_field="reference_value_id",
                parent_id=obj.id,
                translations=translations,
            )
        # `translations` is always in the response schema — refresh it
        # unconditionally, not just when translations were provided, or the
        # relationship is left unloaded (create) / expired-post-commit
        # (update) and response serialization crashes with a MissingGreenlet
        # error trying to lazy-load it outside the async context.
        await self.repository.session.refresh(obj, attribute_names=["translations"])
        return obj

    async def update(self, id: uuid.UUID, obj_in: ReferenceValueUpdate) -> ReferenceValue:
        data = obj_in.model_dump(exclude_unset=True)
        translations = data.pop("translations", None)
        obj = await self.get(id)
        if data:
            obj = await self.repository.update(obj, data)
        if translations:
            await upsert_translations(
                self.repository.session,
                translation_model=ReferenceValueTranslation,
                parent_fk_field="reference_value_id",
                parent_id=obj.id,
                translations=translations,
            )
        await self.repository.session.refresh(obj, attribute_names=["translations"])
        return obj
